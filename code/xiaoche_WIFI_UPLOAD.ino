...
// 本文件 = xiaoche_4_27 + ArduinoOTA 无线烧录。
// 首次须 USB 烧录；开发板菜单「Partition Scheme」须选带 OTA 的（如 Minimal SPIFFS / Huge APP 等，以板卡说明为准）。
// IDE 无线上传：连接同一 LAN 后，工具 -> 端口 -> 选网络端口（密码见下方 OTA_PASSWORD）。
